...
Paciente *retiraFim(Paciente *LISTA) {
  if (LISTA->prox == NULL) {
    ...
    Paciente *ultimo = LISTA->prox, *penultimo = LISTA;
...
int main() {
  ...
  char sResposta = 'S';
    lista = NULL;
  while ((sResposta != 'N')) {
    printf("insira um elemneto\n");
    scanf("%d", &vEntrada);
    lista = inserir_lista(lista, vEntrada);
    getchar();
    printf("Digite S para continuar e N para n\n");
    scanf("%c", &sResposta);
  }
  ...
  while (opcao != 8) {
    printf("\ndigite a opcao:\n1 -excluir no fim\n2 - excluir no inicio\n3 - "
           "excluir um elemento pesquisado\n4 - buscar um elemento na lista");
    scanf("%d", &opcao);
    if (opcao == 1) {
